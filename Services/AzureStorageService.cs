...
namespace Crowdfunding_API.Services
{
    public class AzureStorageService : IFileStorageService
    {
        ...
        public async Task DeleteFiles(string fileRoute, string containerName)
        {
            if (fileRoute != null)
            {
        ...
        {
            await DeleteFiles(fileRoute, containerName);
        ...
        public async Task<string> SaveFile(byte[] content, string extension, string containerName, string contentType)
        {
            var account = CloudStorageAccount.Parse(connectionString);
            var client = account.CreateCloudBlobClient();
            var container = client.GetContainerReference(containerName); // container  = folder
